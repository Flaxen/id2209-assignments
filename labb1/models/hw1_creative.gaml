...
global {
	...
	init {
		...
		create entryExit number:numberOfGates;
...
species entryExit {
	reflex addMoreGuests when: flip(0.01){
		write "new guest arrived";
		create Guest number:1;
		Guest[length(Guest)-1].location <- location;
	}
	
	aspect base {
		rgb agentColor <- rgb("black");
		
		draw hexagon(2) color: agentColor;
	}
...
species Guest skills: [moving] {
	...
	reflex goHome when: targetPoint != nil and flip(0.001) and !bad {
		targetPoint <- entryExit[0].location;
	}
	
	reflex leave when: !empty(entryExit at_distance distanceThreshold) and targetPoint = entryExit[0].location {
		write "going home";
		do die;
	}
	...
	reflex becomeBad when: !empty(Store at_distance distanceThreshold) {
		bool drinksAvailable <- false;
		ask Store at_distance distanceThreshold {
			drinksAvailable <- self.hasDrink;
		}
		
		if(!bad and flip(0.01) and drinksAvailable) {
		bad <- true;
